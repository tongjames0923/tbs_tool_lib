 ...
 * @tparam N 数组大小
 */
template <typename T, size_t N> struct ConstexprValue {
  std::array<T, N> value;
   ...
   * @return 引用数组中的元素
   */
  constexpr T &get(size_t index = 0) { return value[index]; }
   ...
   * @return 常量引用数组中的元素
   */
  constexpr const T &get(size_t index = 0) const { return value[index]; }
 ...
 * @tparam T 元素类型
 */
template <typename T> struct ConstexprValue<T, 1> {
  T value;
 ...
 * @tparam N 字符数组的大小
 */
template <size_t N> struct ConstexprValue<char, N> {
  ...
  template <size_t N1>
  explicit constexpr ConstexprValue(CONST ConstexprValue<char, N1> &other)
      : length(0), value{} {
    if (other.size() >= N) {
  ...
  template <size_t N1>
  constexpr ConstexprValue<char, N1>
  operator=(CONST ConstexprValue<char, N1> &other) CONST {
  ...
  template <size_t N1>
  constexpr ConstexprValue<char, N + N1>
  operator+(CONST ConstexprValue<char, N1> &other) const {
...
template <size_t N> using ConstexprString = ConstexprValue<char, N>;
