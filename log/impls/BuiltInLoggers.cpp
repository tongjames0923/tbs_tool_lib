...
static const char *logLevelToColor[] = {"\x1B[31m",  // 红色
										"\x1B[33m",  // 黄色
										"\x1B[32m",  // 绿色
										"\x1B[36m",  // 青色
										"\x1B[34m"   // 蓝色
...
void BuiltInLoggers::ConsoleLogger::log(const LogLevel &level, const char *str) const {
  ...
  printf("%s", "\x1B[4m");
  printf("%s", str);//输出日志内容
  printf("\x1B[0m %s %s", logLevelToColor[level - 1],
		 std::format(" ;LOG_TIME: [ {}.{} ];", time_utils::outputTime(now),
