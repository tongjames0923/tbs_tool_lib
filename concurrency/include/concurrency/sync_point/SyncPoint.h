 ...
 */
template<size_t SAME_TIME_WAIT_COUNT = 1> class SyncPoint {
private:
	/**
	...
	int _lock_mutex() {
		...
		_threadSafe_condition.wait(lock, [&]() { return !_waiting_mutexs.empty(); });
	...
	void wait(unsigned long long ms,
		...
		if (timeLimited) {
			_conditions[i].wait_for(lock,
			                        time_utils::ms(ms),
			                        [&]() { return predic() || (flagCheck && getFlag() >= target); });
		} else {
			_conditions[i].wait(lock, [&]() { return predic() || (flagCheck && getFlag() >= target); });
		} {
			std::unique_lock<std::mutex> threadLock(_thread_safe_mutex);
			_waiting_mutexs.push(i);
		}
		_threadSafe_condition.notify_all();
	}
...
public:
	...
	void wait_to_predicate(__predic_function f) {
		wait(0, [f]() { return f(); }, false, -1, false);
	}
	/**
	 *  等待直到指定的现代谓词函数返回 true。
	 * @param f
