...
#define ANY_MATCH(f) make_any_matcher([&](auto _target_,auto _v_)->auto{ \
f                                                                         \
} )

// 定义一个宏，用于创建一个可以匹配特定值的匹配器
#define EQUAL_MATCH(val, f) make_equal_matcher(val,[&](auto _target_,auto _v_)->auto{ \
f                                                                             \
} )

// 定义一个宏，用于创建一个基于函数的匹配器
#define FUNCTION_MATCH(e, f) make_functional_matcher([&](auto _target_,auto _v_)->auto{ f },[&]() {\
return e;\
})
